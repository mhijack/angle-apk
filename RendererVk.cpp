                                     ...
                                     const char *wsiLayer)
{
    ...
    WARN() << "------ vkEnumerateInstanceLayerProperties address: " << &vkEnumerateInstanceLayerProperties;
    WARN() << "------ vkEnumerateInstanceExtensionProperties address: " << &vkEnumerateInstanceExtensionProperties;
    WARN() << "instanceLayerCount after query: " << instanceLayerCount;
    CALLED_FUNC_LOG();
    std::vector<VkLayerProperties> instanceLayerProps(instanceLayerCount);
    if (instanceLayerCount > 0)
    {
        ANGLE_SCOPED_DISABLE_LSAN();
        ANGLE_VK_TRY(displayVk, vkEnumerateInstanceLayerProperties(&instanceLayerCount, instanceLayerProps.data()));
    }
    CALLED_FUNC_LOG();
    ...
    {
        ANGLE_SCOPED_DISABLE_LSAN();
        ANGLE_VK_TRY(displayVk, vkEnumerateInstanceExtensionProperties(nullptr, &instanceExtensionCount, instanceExtensionProps.data()));
    }
    WARN() << "instanceExtensionCount after query: " << instanceExtensionCount;
    CALLED_FUNC_LOG();
    // Enumerate instance extensions that are provided by explicit layers.
    for (const char *layerName : enabledInstanceLayerNames)
    {
        WARN() << "instance extension layerName: " << *layerName;
        uint32_t previousExtensionCount      = static_cast<uint32_t>(instanceExtensionProps.size());
        uint32_t instanceLayerExtensionCount = 0;
        {
            ANGLE_SCOPED_DISABLE_LSAN();
            ANGLE_VK_TRY(displayVk, vkEnumerateInstanceExtensionProperties(layerName, &instanceLayerExtensionCount, nullptr));
        }
        instanceExtensionProps.resize(previousExtensionCount + instanceLayerExtensionCount);
        {
            ANGLE_SCOPED_DISABLE_LSAN();
            ANGLE_VK_TRY(displayVk, vkEnumerateInstanceExtensionProperties(layerName, &instanceLayerExtensionCount, instanceExtensionProps.data() + previousExtensionCount));
        }
    }
    CALLED_FUNC_LOG();
    ...
    ANGLE_VK_TRY(displayVk, VerifyExtensionsPresent(instanceExtensionNames, enabledInstanceExtensions));
...
#    endif  // !defined(ANGLE_SHARED_LIBVULKAN)
    ...
    {
        vk::AddToPNextChain(&createInfo, &mProtectedMemoryFeatures);
    }
                              ...
                              const vk::ExtensionNameList &deviceExtensionNames)
{
    ...
    ANGLE_FEATURE_CONDITION(
        ...
        IsAndroid() &&
            ...
            &&
            ExtensionFound(VK_KHR_EXTERNAL_MEMORY_EXTENSION_NAME, deviceExtensionNames));
#endif
